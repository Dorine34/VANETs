...
public class HighwayTower extends Node {
	...
	public void onClock() {
		...
		int tourUti=MainProgDyn.tourUtilisee(this.getID());
		if(l_utilisateurs.size()>0)
		{
			...
			if (tourUti!=-1)
			{
				MainProgDyn.vectDebitTotal.remove(tourUti);
			}

			for (int i=0; i<MainProgDyn.vectDebitTotal.size();i++)
			{
				MainProgDyn.vectDebitTotal.add(dpt);
		...
		{
			res+=MainProgDyn.vectPassageTemps.get(i).getDebit();
				
		}
		System.out.println("Tour "+this.getID()+" : "+ l_utilisateurs.size()+" ; "+res);
	
		System.out.println("Tour "+this.getID()+" : "+ l_utilisateurs.size());
	}
}